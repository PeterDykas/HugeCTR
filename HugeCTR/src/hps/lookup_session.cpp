...
LookupSession::LookupSession(const InferenceParams& inference_params,
  ...
  // What if we just remove all the checks and try to ignore the device_id of the inference params.
  // Lets create a look up
  // session just based on the embedding cache device id
  ...
  try {
    ...
    //       HCTR_OWN_THROW(
    //           Error_t::WrongInput,
    //           "The device id of inference_params is not consistent with that of embedding
    //           cache.");
    //     }
    HCTR_LOG(INFO, ROOT, "Create inference session on device: %d\n",
             embedding_cache_->get_device_id());
    ...
    CudaDeviceContext context(embedding_cache_->get_device_id());
...
void LookupSession::lookup(const std::vector<const void*>& h_keys_per_table,
  ...
  // Is this cuda device context even used
  CudaDeviceContext context(embedding_cache_->get_device_id());
